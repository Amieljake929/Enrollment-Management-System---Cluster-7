...
class ShsRecordsController extends Controller
{
    public function index()
    {
        // 1. Fetch data from the single API source
        ...
        // 2. Access the 'data' key from the JSON response
        $jsonData = $response->json();
        $allRecords = collect($jsonData['data'] ?? []);

        // 3. Complete SHS Strands List (Matched with API "EnrolledCourse")
        $strands = [
        ...
        foreach ($strands as $code => $fullName) {
            // Filter base sa EnrolledCourse field
            ...
            $groupedData[$code] = $studentsInStrand->groupBy('Section')->map(function ($group) {
                ...
                // Get the Grade Level from the first student in the group for the section summary
        ...
        return view('modules.recordsShs', compact('strands', 'groupedData'));
    }
}