                        ...
                        @foreach($groupedQuestions as $course => $courseQuestions)
                            @if($course !== 'TIE_BREAKER')
                                <div class="mb-5 p-3 border rounded bg-light">
                                    <h5 class="text-primary">{{ $allCourses[$course] ?? $course }}</h5>
                                    <hr>
                            ...
                            @if($q['type'] === 'tie_breaker')
                                <div class="mb-5 p-3 border rounded bg-warning">
                                    ...
                                    @foreach($q['options'] as $opt)
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="answers[{{ $q['id'] }}]" value="{{ $opt['course'] }}" id="tb{{ $q['id'] }}_{{ $opt['label'] }}" required>
